...
def fatraols():
     ...
     for i in info:
          ...
          atdrkod=mcmb3.get() 
          odeme=int(grs13.get())
          atutar=amktar*amalfyat
          kmt.execute("insert into islemals values(@p1,@p2,@p3,@p4,@p5,@p6,@p7,@p8)",
                      (iskod,amalkod,amalad,amalfyat,amktar,atutar,atdrkod,odeme))
     bglnl.commit()
     bglnl.close
...
etk=Label(frm,text="Tedarikçi adı",font=("Bold",10)
          #width= 20,
    ).place(x=290,y=36)
...
grs12=Label(satsekr,text="Tedarikçi ad").place(x=280,y=1)
...
grs23=Label(satsekr,text="Alış fiyat",font=("Bold",10)
          #width= 20,
   ).place(x=255,y=55)
...
kmt1=bglnl.cursor()
kmt1.execute("select tedarkad from tedark group by tedarkad")
